# Executor Execution Result Event 远端验证设计

## 1. 文档目标

本文档定义如何在主服务器对 `executor.execution_result` 做一次与本地实现解耦、
且可重复执行的远端闭环验证，确认 executor 在真实消费执行消息时，能够把
`spot_arbitrage_probe` richer result 正确发成结构化运行时事件。

本次目标是在不触发真实交易所下单、不依赖现网 systemd 服务状态、也不扩大到新的
数据库持久化设计的前提下，验证以下事实：

- `OPEN_HEDGED` rich result 会发出 `executor.execution_result`
- `OPEN_PARTIAL` rich result 会发出 `executor.execution_result`
- preflight 失败不会误发 `executor.execution_result`
...
- 为 `executor.execution_result` 新增主服务器远端验证 helper
...
- [live_workers.py](file:///d:/old/FuRunSystemV4/app/runtime/live_workers.py)
  中 `RedisExecutionTaskConsumer` 已新增 `executor.execution_result`
- [test_live_workers.py](file:///d:/old/FuRunSystemV4/tests/test_live_workers.py)
  已通过本地 TDD 锁定：
  - rich `OPEN_HEDGED` 会发事件
  - rich `OPEN_PARTIAL` 会发事件
  - summary-only 结果对象也能兼容发事件
  - preflight 失败不会发事件
- [live-workers-systemd.md](file:///d:/old/FuRunSystemV4/docs/ops/live-workers-systemd.md)
  已沉淀过：
  - task-account-binding 远端验证
  - execution result summary 远端验证
  - spot probe richer result 远端验证

但当前仍有一个缺口：

1. `executor.execution_result` 只在本地测试里通过，还没有主服务器闭环记录
2. 运维文档中还没有这类事件的远端验证章节
3. 后续如果要继续接 repair / 报表消费层，缺少一次“远端事件确实能发出来”的基线
...
2. 运维文档缺少 `executor.execution_result` 的复跑步骤
...
- 向远端执行流写 canary 消息
- 从 `journalctl` 中抓取 `executor.execution_result`
...
### 6.3 方案 C：只在远端调用 `_build_executor_execution_result_event(...)`

做法：

- 直接 import builder
- 构造 payload 和 result，校验返回 `RuntimeEvent`
...
- 它在成本与真实性之间最平衡
- 它能验证 `dispatch -> execution_status -> event_router.dispatch(...)` 的完整链路
- 它与此前 execution summary / richer result 的远端 helper 路线一致
...
- 一条合法执行消息
- `FakeSpotService` 返回 rich `OPEN_HEDGED`

断言：

- `processed = 1`
- 存在 1 条 `executor.execution_result`
- 该事件至少包含：
...
- 一条合法执行消息
- `FakeSpotService` 返回 rich `OPEN_PARTIAL`
...
- 仍可看到失败路径被处理，但不把 preflight 误当成真实执行结果
...
5. `FakeSpotService` 返回预设 rich result
...
- 如果 helper 执行报 import 或环境错误：
  - 先视为远端环境问题
  - 不直接判定业务逻辑失败
- 如果 `rich_hedged / rich_partial` 未产生 `executor.execution_result`
  - 视为本次功能远端闭环失败
- 如果 preflight 场景产生了 `executor.execution_result`
  - 视为严重语义回归
...
- 运行本地同步脚本
- 获取 `rich_hedged / rich_partial / preflight` 三段 JSON 结果
- 对照 spec 中的字段断言人工复核
...
1. 主服务器 helper 能成功运行
2. `rich_hedged` 场景出现 `executor.execution_result`
3. `rich_partial` 场景出现 `executor.execution_result`
4. preflight 场景不出现 `executor.execution_result`
5. 远端输出字段与本地测试契约一致
6. 运维文档完成记录
...
- 让 repair 或运维报表直接消费 `executor.execution_result`
- 把部分 richer result 字段继续下沉到任务表
